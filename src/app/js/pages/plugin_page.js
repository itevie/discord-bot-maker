...
export default function init() {
    ...
    document.getElementById("plugin-search").addEventListener("keyup", () => {
        render(document.getElementById("plugin-search").value);
    });
    ...
    if (query) {
        for (const plugin in queried) {
            ...
            if (testQuery([plugins[plugin].name, plugins[plugin].author], query))
                keepPlugin = true;
            if (!keepPlugin)
                delete queried[plugin];
        }
    }
    // Fucking monstor of a whatever the fuck this is
    const container = createContainer(iterate(queried, (pluginKey, pluginValue) => createBaseContainer([
        ...
        createContainer([
            ...
            !pluginValue.languageFeatures ? null : createBaseContainer([
                ...
                createContainer(iterate(pluginValue.languageFeatures, (featureKey, featureValue) => createBaseContainer([
                    createLabel(featureKey),
                    createContainer(iterate(featureValue.values, (valueKey, valueValue) => createBaseContainer([
...
function testQuery(arrays, query) {
...
function createBaseContainer(elements) {
