// This entire file was stolen from Syrenity 2.1
...
export function registerContextMenu(data: ContextMenuDetails) {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!In(e.clientX, e.clientY, contextContainer)) {
...
function showContextMenu(element: HTMLElement, contextMenu: ContextMenuDetails, event: MouseEvent = null) {
  ...
  let coordinates = event ? { x: event.clientX, y: event.clientY } : getFor(element);
  ...
  coordinates = bound(coordinates.x, coordinates.y, contextContainer);
 ...
 */
function bound (x: number, y: number, element: HTMLElement): Coordinate {
 ...
 */
function In(x: number, y: number, element: HTMLElement): boolean {
